...
//             <input type="text" placeholder={tForm('buttonNamePlaceholder')} {...register(`choices.${index}` as const)} className="w-full max-w-xs bg-gray-200 input-sm input focus:outline-0" />
//             {errors?.choices && <p className='justify-start mb-1 text-xs text-error'>{errors?.choices?.message}</p>}
//             <input type="text" placeholder={tForm('urlPlaceholder')} {...register(`choiceLinks.${index}` as const)} className="w-full max-w-xs bg-gray-200 input-sm input focus:outline-0" />
