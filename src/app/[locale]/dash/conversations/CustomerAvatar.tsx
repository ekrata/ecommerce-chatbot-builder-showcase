...
export const CustomerAvatar: React.FC<Props> = ({ conversationItem, message }) => {
  return (
    <div className={`avatar w-12 h-12 bg-${getRandomColor(conversationItem?.conversation.customerId ?? '')}-500 background rounded-full p-2 ring-2 ring-primary ${conversationItem?.conversation.type === 'botChat' && 'online'} ${message?.sender === 'operator' && conversationItem?.conversation.operator.online ? 'online' : 'offline'}`}>
