...
import {
  MessengerEvent,
  MetaEvent,
} from '@/packages/functions/app/api/src/meta/metaEvents';
...
export function baseStack({ stack, app }: StackContext) {
  ...
  const metaWhatsappTopicSubs:
    | Record<
      ...
      >
    | undefined = {
    [MetaEvent.WhatsappMessages]: {
      type: 'queue',
      queue: new Queue(stack, `meta_${MetaEvent.WhatsappMessages}_queue`, {
        cdk: defaultQueueConfig,
        consumer: {
          function: {
            timeout: defaultFunctionTimeout,
            bind: [wsApi, api, REGION, table],
            handler:
              'packages/functions/app/api/src/meta/whatsapp/webhooks/messages.handler',
          },
        },
      }),
      cdk: {
        subscription: {
          filterPolicy: {
            type: SubscriptionFilter.stringFilter({
              allowlist: [MetaEvent.WhatsappMessages],
            }),
          },
        },
      },
  ...
  metaWhatsappTopic.addSubscribers(stack, metaWhatsappTopicSubs);

  const metaMessengerTopicSubs:
    | Record<
        string,
        | FunctionInlineDefinition
        | Queue
      ...
      >
    | undefined = {
    [MetaEvent.MessengerMessages]: {
      type: 'queue',
      queue: new Queue(
        stack,
        `meta_messenger_${MetaEvent.MessengerMessages}_queue`,
        {
          consumer:
      ...
      cdk: {
        subscription: {
          filterPolicy: {
            type: SubscriptionFilter.stringFilter({
              allowlist: [MetaEvent.MessengerMessages],
            }),
          },
        },
      },
